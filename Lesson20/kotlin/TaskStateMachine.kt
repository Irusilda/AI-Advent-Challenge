...
class TaskStateMachine {
    ...
    fun transitionTo(newStage: Stage, reason: String = ""): TransitionResult {
        ...
        state = when (newStage) {
            ...
            Stage.EXECUTION -> state.copy(
                ...
                currentStep = if (oldStage == Stage.PLAN_APPROVED) 0 else state.currentStep,
    ...
    fun setPlan(plan: String): Boolean {
        if (state.stage in setOf(Stage.INIT, Stage.PLANNING, Stage.EXECUTION, Stage.VALIDATION)) {
            if (state.stage != Stage.PLANNING) {
                val planningResult = transitionTo(Stage.PLANNING, "Перепланирование")
                if (planningResult !is TransitionResult.Success) return false
            }
            val stepCount = plan.lines().size.coerceIn(5, 100)
            state = state.copy(plan = plan, totalSteps = stepCount)
     ...
     * Единый метод вместо дублирования executeStep/executeNextStep.
     * Возвращает true, если шаг выполнен успешно.
     * Если все шаги выполнены — автоматически переходит к VALIDATION.
     */
    fun executeNextStep(): Boolean {
        ...
        state = state.copy(
            ...
            expectedAction = if (stepNumber < state.totalSteps)
                "выполнить шаг ${stepNumber + 1}"
            else
                "завершить выполнение"
        )

        stepHistory.add("Шаг $stepNumber выполнен")
