...
class ChatAgent(
    ...
    private suspend fun processWithMcpTools(
        ...
        val toolsResult = mcp.connect().listTools()
        val toolSchemas = toolsResult.tools().map { tool ->
            ToolSchema(
                name = tool.name(),
        ...
        val mcpInstruction = """
        У тебя есть доступ к MCP-инструментам для погоды. Ты обязан их использовать, когда пользователь спрашивает о погоде.
        НЕЛЬЗЯ отвечать на вопросы о погоде из памяти или контекста — всегда вызывай инструмент.
        Даже если ты думаешь, что знаешь ответ, всё равно вызови инструмент.
        """.trimIndent()
        ...
        var iterations = 0
        while (response.toolCalls != null && response.toolCalls.isNotEmpty() && iterations < 10) {
            iterations++

            val assistantJson = JSONObject().apply {
                put("role", "assistant")
                put("content", JSONObject.NULL)
                put("tool_calls", buildToolCallsJson(response.toolCalls))
            }
            ...
            for (tc in response.toolCalls) {
                ...
                val result = mcp.callTool(tc.function.name, args)
