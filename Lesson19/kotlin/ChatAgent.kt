...
class ChatAgent(
    ...
    init {
        if (enableSwarm) {
            orchestrator = AgentOrchestrator(llm, taskState)
            println("🐝 Рой агентов инициализирован")
        }
    ...
    private suspend fun processMessageInternal(userMessage: String): DeepSeekResponse {
        // Проверяем специальные команды для роя
        when (userMessage.lowercase()) {
            ...
            "next", "дальше" -> {
                ...
                return if (taskState.executeStep()) {
        ...
        // Если рой активирован, пробуем обработать через него
        if (orchestrator != null) {
            val swarmResult = orchestrator?.processUserRequest(userMessage)

            if (swarmResult != null && !swarmResult.startsWith("❌") && !swarmResult.startsWith("⚠️")) {
                return createEmptyResponse("🐝 [Рой агентов]\n$swarmResult")
            } else if (swarmResult != null) {
    ...
    private suspend fun processRegularMessage(userMessage: String): DeepSeekResponse {
        ...
        // Без роя — свободный диалог, LLM + MCP, без этапов
        if (orchestrator == null) {
    ...
    private suspend fun processNormalMessage(userMessage: String): DeepSeekResponse {
        ...
        // Фоновый расчет полной статистики
        scope.launch {
    ...
    private suspend fun processWithMcpTools(
        ...
        val mcp = mcpClientManager!!
        val toolSchemas = mcp.listAllTools().map { tool ->
            ToolSchema(
                name = tool.name(),
        ...
        val toolList = toolSchemas.joinToString(", ") { it.name }
        val mcpInstruction = """
        У тебя есть доступ к MCP-инструментам: $toolList.
        Ты ОБЯЗАН их использовать для соответствующих запросов.
        НЕЛЬЗЯ отвечать на вопросы из памяти или контекста, если подходящий инструмент доступен — всегда вызывай его.
        ...
        Для разовой сводки: get-aggregated.
        ...
        val fullSystem = "${systemMessage.content}\n\n$mcpInstruction"

        val messagesJson = JSONArray().apply {
            put(JSONObject().put("role", systemMessage.role).put("content", fullSystem))
            initialMessages.filter { it.role != "system" }.forEach { msg ->
                put(JSONObject().put("role", msg.role).put("content", msg.content))
            }
            put(JSONObject().put("role", userMessage.role).put("content", userMessage.content))
        }

        var response = withContext(Dispatchers.IO) {
            llm.askJson(messagesJson, tools = toolSchemas)
        }

        var iterations = 0
        val toolLoop: suspend () -> Unit = {
            while (iterations < 3) {
                ...
                for (tc in toolCalls) {
                    val result = try {
                        ...
                        mcp.callTool(tc.function.name, args)
                    } catch (e: Exception) {
                        "Error: ${e.message}"
                    }
        ...
        val taskKeywords = listOf("напомин", "запланируй", "каждую минут", "каждые", "сводк", "показывай погоду", "периодическ")
        if (iterations == 0 && taskKeywords.any { userMessage.content.lowercase().contains(it) }) {
            messagesJson.put(JSONObject().apply {
                put("role", "user")
                put("content", "[SYSTEM] Ты не вызвала инструмент. Пользователь просит запланировать задачу. Используй schedule-task или schedule-mcp-tool и напиши ТОЛЬКО то, что вернул инструмент. Не придумывай ID задач.")
            })
            response = withContext(Dispatchers.IO) {
                llm.askJson(messagesJson, tools = toolSchemas)
            }
        ...
        var autoSummaryCreated = false
        if ((userMessage.content.lowercase().contains("сводк") || userMessage.content.lowercase().contains("отчет")) && iterations > 0) {
            var collectName: String? = null
            var collectInterval = 1
            for (i in (0 until messagesJson.length()).reversed()) {
                val msg = messagesJson.optJSONObject(i) ?: continue
                if (msg.optString("role") == "assistant") {
                    val tcs = msg.optJSONArray("tool_calls") ?: continue
                    for (j in 0 until tcs.length()) {
                        val tc = tcs.getJSONObject(j)
                        val func = tc.optJSONObject("function") ?: continue
                        if (func.optString("name") == "schedule-mcp-tool") {
                            try {
                                val argsJson = JSONObject(func.optString("arguments"))
                                val n = argsJson.optString("name", null)
                                if (n != null) { collectName = n; collectInterval = argsJson.optInt("interval_minutes", 1) }
                            } catch (_: Exception) {}
                        }
                    }
                    if (collectName != null) break
                }
            }
            if (collectName != null) {
                val existingTasks = mcp.callTool("list-tasks", emptyMap())
                val summaryTaskName = "summary-$collectName"
                if (!existingTasks.contains(summaryTaskName)) {
                    val summaryArgs = linkedMapOf<String, Any>(
                        "name" to summaryTaskName,
                        "type" to "summary",
                        "interval_minutes" to collectInterval,
                        "params" to mapOf("source_task_name" to collectName)
                    )
                    mcp.callTool("schedule-task", summaryArgs)
                    autoSummaryCreated = true
                }
            }
        }

        val safeContent = response.content.takeIf { it.isNotBlank() }
            ?: if (autoSummaryCreated) "✅ Сводка настроена, результаты появятся в чате."
            else "(empty response)"
        val assistantMsg = Message("assistant", safeContent)
        addAssistantMessage(assistantMsg)
        ...
        return response.copy(content = safeContent)
    }
