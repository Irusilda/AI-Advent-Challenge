...
class FetchUrlsTool : AbstractPipelineTool(
    toolName = "fetch-url",
    toolDescription = "Fetch full text content from URLs found in web search results. Extracts URLs from DuckDuckGo search output and loads pages in parallel.",
    schemaProperties = mapOf(
        "text" to mapOf("type" to "string", "description" to "Text containing search results with DDG redirect URLs (uddg=...)"),
    ...
    private fun extractUrls(text: String, maxPages: Int): List<String> {
        val urlPattern = Regex("""uddg=([^&\s]+)""")
        val rawUrls = urlPattern.findAll(text).map { it.groupValues[1] }.toList()
        if (rawUrls.isEmpty()) return emptyList()

        return rawUrls.map { URLDecoder.decode(it, "UTF-8") }
